import pytest

from datastructures import LinkedList


def test_insert():
    linked_list = LinkedList()
    assert linked_list.first is None
    assert linked_list.last is None
    linked_list.insert('a')
    assert linked_list.first.item == 'a'
    assert linked_list.first.next is None
    assert linked_list.last.item == 'a'
    linked_list.insert('b')
    assert linked_list.first.item == 'b'
    assert linked_list.first.next.item == 'a'
    assert linked_list.last.item == 'a'


def test_search():
    linked_list = LinkedList()
    for i in 'abc':
        linked_list.insert(i)
    assert linked_list.search('a').item == 'a'
    assert linked_list.search('b').item == 'b'
    assert linked_list.search('c').item == 'c'


def test_search_fail():
    linked_list = LinkedList()
    for i in 'abc':
        linked_list.insert(i)
    with pytest.raises(FileNotFoundError):
        linked_list.search('d')


def test_previous():
    linked_list = LinkedList()
    for i in 'abc':
        linked_list.insert(i)
    assert linked_list.previous('a').item == 'b'
    assert linked_list.previous('b').item == 'c'
    assert linked_list.previous('c') is None


def test_delete():
    linked_list = LinkedList()
    for i in 'abc':
        linked_list.insert(i)
    linked_list.delete('b')
    assert linked_list.first.item == 'c'
    assert linked_list.first.next.item == 'a'
    assert linked_list.last.item == 'a'
    linked_list.delete('a')
    assert linked_list.first.item == 'c'
    assert linked_list.first.next is None
    assert linked_list.last.item == 'c'
    linked_list.delete('c')
    assert linked_list.first is None
    assert linked_list.last is None


def test_delete_fail():
    linked_list = LinkedList()
    for i in 'abc':
        linked_list.insert(i)
    with pytest.raises(FileNotFoundError):
        linked_list.delete('d')
